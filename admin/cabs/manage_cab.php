...
<div class="card card-outline card-purple rounded-0">
	...
	<div class="card-body">
		<form action="" id="cab-form">
			...
			<div class="form-group">
    <label for="car_model_image" class="control-label">Car Model Image</label>
    <div class="custom-file">
        <input type="file" class="custom-file-input rounded-circle" id="car_model_image" name="car_model_image" onchange="displayImg(this, $(this))">
        <label class="custom-file-label" for="car_model_image">Choose file</label>
...
<?php
if (isset($_GET['id']) && $_GET['id'] > 0) {
    // Existing cab data, so it's an update
    ...
    if ($_FILES['car_model_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExtension, $allowedExtensions)) {
            ...
            if (move_uploaded_file($_FILES['car_model_image']['tmp_name'], $targetFile)) {
                ...
                // Insert or update the file path in the database
                if (isset($id) && $id > 0) {
                    // This is an update, so update the existing record
                    $sql = "UPDATE cab_list SET car_model_image_path = ? WHERE id = ?";
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("si", $carModelImagePath, $id);
                    $stmt->execute();
                } else {
                    // This is an insert, so insert a new record
                    $sql = "INSERT INTO cab_list (car_model_image_path) VALUES (?)";
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("s", $carModelImagePath);
                    $stmt->execute();
                }

                echo "File uploaded successfully and path inserted or updated in the database.";
            } else {
                echo "Sorry, there was an error uploading your file.";
            }
        } else {
...
?>
			...
			<div class="form-group">
				<label for="body_no" class="control-label">price#</label>
